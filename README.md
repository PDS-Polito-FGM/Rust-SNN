...
- ### Builder
  The `Builder` module allows you to actually create effectively the structure of
  ...
  - #### SnnBuilder (Spiking Neural Network Builder)

    The `SnnBuilder` allows to create a `Spiking Neural Network` statically in which each layer is composed by static vectors of neurons,
    weights and intralayer weights. The compiler can check the correctness of the network structure before the execution but this implies that all
    the structure of the network is allocated on the **Stack** (**Not fitting with large networks**).

  - #### DynSnnBuilder (Dynamic Spiking Neural Network Builder)
    The `DynSnnBuilder` allows to create a `Spiking Neural Network` dynamically in which each layer is composed by dynamic vectors of neurons,
    weights and intralayer weights. The compiler can not check the correctness of the network structure before the execution but this implies that all
    the structure of the network is allocated on the **Heap** (**Fitting with large networks**).
...
- ### Network
  ...
  - #### Snn (Spiking Neural Network)
    ...
    As the `SnnBuilder` the `Snn` receive the input as a static vector of spikes and the output is a static vector as well.
  ...
  - #### DynSnn (Dynamic Spiking Neural Network)
    ...
    As the `DynSnnBuilder` the `DynSnn` receive the input as a dynamic vector of spikes and the output is a dynamic vector as well.
    The correctness of the input can be checked only at run time.

## Main structures
The library provides the following main structures:

- `LifNeuron` represents a neuron of the `Leaky Integrate and Fire` type, it can be used to build a `Layer` of neurons. 
   ...
   - #### `SnnBuilder` methods:
     ...
     -  **weights()** method:
        ...
        ```rust
          ...
          ```
          adds weights between the layers of the `SnnBuilder` passed as parameters
       ...
       - **neurons()** method:
         ...
         ```
          adds neurons to the layer of the `SnnBuilder` passed as parameters
     - **intra_weights()** method
        ```rust
         ...
         adds intra-weights to the layer of the `SnnBuilder` passed as parameters
     ...
     - **build()** method:
        ...
        ```rust
         ...
         builds the `SNN` from the `SnnBuilder`
...
- #### `DynSnnBuilder` methods:
   ...
   - **add_layer()** method:
     ...
     adds a new layer to the `DynSnnBuilder` with the given neurons, weights and intra_weights passed as parameters
   ...
   - **build()** method:
     ...
     builds the `DynSNN` from the `DynSnnBuilder`
 ...
 - ### Network Methods
   - #### `Snn` method:
      - process() method:
        ...
        processes the input passed as parameter and returns the output of the network
   - #### `DynSnn` method:
        - process() method:
...
## Usage examples
The following example shows how to create a `Spiking Neural Network` with the `SnnBuilder` and how to execute it on a given input.
...
```
The following example shows how to create a `Spiking Neural Network` with the `DynSnnBuilder` and how to execute it on a given input.
